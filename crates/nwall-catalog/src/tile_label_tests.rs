...
use super::*;
    ...
    fn search_query_becomes_caption() {
        ...
        assert_eq!(it.tile_label(), "Matterhorn");
        assert!(it.tooltip_label().contains("1920×1080 · General · #zztest2"));
    }
    ...
    fn stats_format_skips_unknown() {
        ...
        assert!(t.contains("Size: 2.3 MB") || t.contains("Size: 2.4 MB"), "{t}");
    ...
    fn wallhaven_search_hit_maps_views_favorites_link_colors() {
        ...
        assert_eq!(it.colors, vec!["#000000".to_string(), "#abbcda".to_string()]);
    ...
    fn wallhaven_avatar_prefers_32px() {
        ...
        assert_eq!(wallhaven_avatar_url(&av).as_deref(), Some("https://example/32.jpg"));
        let w = WallhavenItem {
            id: "abc123".into(),
    ...
    fn tag_file_stem_sanitizes() {
        ...
        assert_eq!(tag_file_stem(&tags, 3).as_deref(), Some("matterhorn-alps-snow"));
        assert_eq!(
            sanitize_file_stem("foo/bar · baz.jpg"),
            "foo-bar-baz-jpg"
        );
    ...
    fn github_direct_child_and_space_encoding() {
        assert!(github_path_is_direct_child("Live Wallpapers/giphy.gif", "Live Wallpapers"));
        assert!(!github_path_is_direct_child(
            "Live Wallpapers/nested/x.gif",
            "Live Wallpapers"
        ));
        assert!(github_path_is_direct_child("Cherry Blossoms/a.jpg", "Cherry Blossoms"));
        assert!(github_path_is_direct_child("root.png", ""));
        assert!(!github_path_is_direct_child("dir/root.png", ""));
        assert!(github_path_is_direct_child("animated/misc/a.mp4", "animated/misc"));
        assert_eq!(
            percent_encode_path("Live Wallpapers/giphy.gif"),
            "Live%20Wallpapers/giphy.gif"
        );
        assert_eq!(
            percent_encode_path("Cherry Blossoms"),
            "Cherry%20Blossoms"
        );
    ...
    fn filter_github_repos_and_images_before_videos() {
        ...
        assert!(only_usman.iter().all(|i| {
            i.repo.as_deref() == Some("usman-369/wallpapers")
        }));
    ...
    fn live_github_builtin_pack_lists_items() {
        ...
        eprintln!("builtin GitHub pack: {} repos → {} items", src.repos.len(), items.len());
        assert!(
            items.len() >= 100,
    ...
    fn bing_item_fills_preview_stats_fields() {
        ...
        let it = bing_item(BingImage {
            ...
            copyright: "Winding road of Julier Pass, Switzerland (© Westend61/Getty Images)"
                .into(),
            copyrightlink:
                "https://www.bing.com/search?q=Julier+Pass+Switzerland&form=hpcapt".into(),
    ...
    fn nasa_item_fills_preview_stats_fields() {
        ...
        assert_eq!(
            nasa_file_type_from_url(
                "https://apod.nasa.gov/apod/image/2608/example_hd.jpg?foo=1"
            )
            .as_deref(),
            Some("JPEG")
        );
        assert_eq!(
            nasa_youtube_watch_url("https://www.youtube.com/embed/1R5QqhPq1Ik?rel=0")
                .as_deref(),
        ...
        assert_eq!(it.url, "https://apod.nasa.gov/apod/image/1410/vela_2000.jpg");
        assert_eq!(
            it.thumb.as_deref(),
    ...
    fn search_cache_rejects_incomplete_bing_nasa() {
        let stale_bing = FetchResult::unpaged(vec![RemoteItem {
            ...
            credit: Some(
                "Winding road of Julier Pass, Switzerland (© Westend61/Getty Images)".into(),
            ),
            ..Default::default()
        }]);
